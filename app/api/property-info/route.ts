...
// Helper function to delay execution (replacement for deprecated waitForTimeout)
...
export async function POST(request: NextRequest) {
  ...
  try {
    const { city, streetName, addressNumber } = await request.json()
    ...
    // Typically: city/town is first, street is second, address is third
    ...
    // Select city - find matching option value
    ...
    // Wait for street dropdown to populate (ASP.NET postback or AJAX)
    const streetSelector = streetSelect.id ? `#${streetSelect.id}` : `select[name="${streetSelect.name}"]`
    
    try {
      // Wait for dropdown to have more than just the default option
      ...
      await delay(3000) // Give it a bit more time
    }
    
    
    // Check if street select exists
    ...
    // Get updated street options after city selection
    // Use $eval for a single element, not $$eval
    ...
    // Check if address select exists
    ...
    // Get updated address options after street selection
    // Use $eval for a single element, not $$eval
    ...
    // Parse the property information from the result page
    const propertyInfo: PropertyInfo = {}
    

    // Extract property details - adjust selectors based on actual HTML structure
    // Try multiple strategies to find the data
    const pageText = $('body').text()
    
    // Strategy 1: Look for data in tables
    ...
    // Also try extracting from page text using regex patterns
    ...
    return NextResponse.json({ success: true, data: propertyInfo })
  } catch (error: any) {
    
    // Return a more helpful error message
    const errorMessage = error.message || 'Failed to fetch property information'
