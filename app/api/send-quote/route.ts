...
// Initialize Resend - check for API key
...
`
    ...
    const recipientEmail = 'admin@valorhvacma.com'
    ...
    if (resend && resendApiKey) {
      try {
        ...
        // Don't fall through - return error so we know it failed
        return NextResponse.json(
          { 
            success: false,
    ...
    // Fallback: Return success even if email service is not configured
    return NextResponse.json({ 
      success: false,
