...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/property/info`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.error || 'Failed to fetch property information' },
    ...
    console.error('Property info proxy error:', error)
    return NextResponse.json(
